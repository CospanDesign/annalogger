...
struct HttpBlob nullBlob = {0, NULL};
...
{ 
  ...
  if (line.uLength == 0)
  {
    HttpDebug("Print: WebSocketResponse\r\n");
    g_state.connections[uConnection].connectionState = WebSocketResponse;
    return 1;
  }
  ...
  if(Origin == 0){
    if (HttpString_nextToken(WS_ORIGIN, sizeof(WS_ORIGIN)-1, line) == line.pData){
          ...
          WS_ORIGIN_NAME = (char *) calloc( line.uLength+1, sizeof(char));
...
static int HttpCore_HandleRequestData(UINT16 uConnection, struct HttpBlob* pData)
{
    ...
    {
        HttpDebug("Received content. Length=%d, content:\r\n%.*s \r\n", g_state.connections[uConnection].request.requestContent.uLength, g_state.connections[uConnection].request.requestContent.uLength, g_state.connections[uConnection].headerStart);
...
int WSCore_HandshakeResponse(UINT16 uConnection, struct HttpBlob line)
{
    ...
    //HttpDebug("preparing websocket response\r\n");
    //HttpDebug("origin: %s\r\n", WS_ORIGIN_NAME);
